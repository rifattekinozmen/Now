...
- [x] Bilanço / gelir tablosu **operasyonel özeti** (dönem içi yevmiye): `BalanceSheetService` + `admin.finance.balance-sheet` — hesap tipine göre özet; **yasal denetim / TFRS tablosu değildir**; açılış-kapanış bakiyesi olmadan tam bilanço iddiası yok _(2026-03-30)_
- [x] **Açılış bakiyeleri + bilanço özeti birleşimi (operasyonel)** — `fiscal_opening_balances` + `LegalFinancialStatementsService` + `admin.finance.fiscal-opening-balances.index` + bilanço özetinde “açılışları dahil et”; **resmi/yasal tablo ve denetim çıktısı değildir** — TFRS ve uzman onayı ayrı süreç _(UI + entegrasyon: 2026-03-30)_
...
### Gerçek durum notu (depo senkronu — 2026-04-20)

Bu dokümandaki §10 checklist çekirdek teslimatları özetler. **Güncel sprint durumu ve sayfa/envanter eşlemesi** için [erp_todos.md](erp_todos.md) ve [views-port-checklist.md](views-port-checklist.md) referans alınmalıdır. Üst bölümlerdeki enterprise / vizyon maddeleri (ör. gelişmiş harita, heatmap, tam B2B portal faturaları) ayrı önceliklendirme ile yol haritasına ayrılır; tek sürümde tamamlanması beklenmez.
