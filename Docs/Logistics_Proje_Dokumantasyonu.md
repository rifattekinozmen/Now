...
- [ ] `User` & `Role` (Spatie) yetki matrisinin tanımlanması.
- [x] `Vehicle` (plaka, şase alanı `vin`, muayene tarihi) — temel model + admin UI _(tamamlandı: 2026-03-28)_
- [ ] `Employee` (T.C., Kan Grubu, SRC) modelleri.
- [ ] Belge Vade Alarm Servisi (`30-15-7-1` kuralı) prototipleme.

### **Faz 2: Lojistik Operasyon (Core Logistics)**
- [x] `Order` & `Shipment` tabloları — temel alanlar + `orders.sas_no`, `delivery_numbers` (PIN havuzu, durum: available/assigned/used), admin UI (`orders-index`, `delivery-numbers-index`); atamada sipariş `sas_no` boşsa PIN üzerindeki SAS kopyalanır. Rutubet / QR / mobil POD yok _(kısmi: 2026-03-30)_
...
- [ ] QR Kod Üretimi ve Mobil POD (Dijital İmza) akışı.

### **Faz 3: Finansal Entegrasyon (Financial Intelligence)**
- [ ] `BankaOCRService` (Ekstre Okuma & Auto-Matching).
- [ ] `LogoERPService` (XML Connect/API Çıktısı).
- [ ] Vadeli Ödeme Takvimi ve Nakit Akış Projeksiyonu.
- [ ] `AuditAI` (Fuel Anomaly, iPOD, Fatigue AI).
